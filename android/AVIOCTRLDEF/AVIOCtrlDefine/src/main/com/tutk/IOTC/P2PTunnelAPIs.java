...
public class P2PTunnelAPIs {
	...
	* can't be connected anymore until anyone session release. */
	...
	public native int  P2PTunnelAgent_Connect_Parallel(String UID, int SID, byte[] authData, int authDataLength, int[] errFromDeviceCB);
	public native int  P2PTunnelAgent_GetFreeSession();
	public native int  P2PTunnelAgent_Connect_Stop(String UID);
	public native int  P2PTunnelAgent_Connect_Stop_Parallel(String UID, int SID);
	...
	public native void P2PTunnelAgent_Set_Log_Path(String path, int nMaxSize);
	...
	public int tunnelSessionInfoCB(sP2PTunnelSessionInfo sSessionInfo) {
		if (mSelf != null) {
			return mSelf.onTunnelSessionInfoChanged(sSessionInfo);
		}
