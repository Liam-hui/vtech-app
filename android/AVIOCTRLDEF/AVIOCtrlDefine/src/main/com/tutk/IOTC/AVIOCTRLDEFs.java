...
public class AVIOCTRLDEFs {
	...
	//For len corrections
    public static final int IOTYPE_USER_IPCAM_GET_LENS_CORRECTION_REQ = 0x781;
    public static final int IOTYPE_USER_IPCAM_GET_LENS_CORRECTION_RESP = 0x782;
    public static final int IOTYPE_USER_IPCAM_SET_LENS_CORRECTION_REQ = 0x783;
    public static final int IOTYPE_USER_IPCAM_SET_LENS_CORRECTION_RESP = 0x784;

	//Nite mode status
    public static final int IOTYPE_VTECH_DAYNIGHTMODE_NOTIFY = 0x785;

	//Set event list favourite
	public static final int IOTYPE_VTECH_SET_FAVOURITEEVENT_REQ = 0x786;
	public static final int IOTYPE_VTECH_SET_FAVOURITEEVENT_RESP = 0x787;
	public static final int IOTYPE_VTECH_IPCAM_LISTEVENT_REQ = 0x788;
	public static final int IOTYPE_VTECH_IPCAM_LISTEVENT_RESP = 0x789;
    ...
    public static final int AVIOCTRL_EVENT_PIR = 0x13;                                                                  // Raymond 2019apr4 skype new type
    public static final int AVIOCTRL_EVENT_RINGBELL = 0x14;                                                     // Raymond 2019apr4 skype new type
    public static final int AVIOCTRL_EVENT_SOUND	 = 0x15;                                                        // Raymond 2019apr4 skype new type
    public static final int AVIOCTRL_EVENT_TEMPERATURE_TOO_HIGH	= 0x16;                     // Raymond 2019apr4 skype new type
    public static final int AVIOCTRL_EVENT_TEMPERATURE_TOO_LOW	= 0x17;                     // Raymond 2019apr4 skype new type
    ...
    public static final int AVIOCTRL_RECORD_PLAY_FASTFORWARD = 0x11;
...
//    public static final int IOTYPE_USER_IPCAM_UPLOAD_FILE_RESP = 0x7F000007;
    ...
    /* Kalay VSaaS Server IOCtrl reference */
    public static final int IOTYPE_USER_IPCAM_SET_VSAAS_PLAYBACK_INFO_REQ = 0x00010000;
    public static final int IOTYPE_USER_IPCAM_SET_VSAAS_PLAYBACK_INFO_RESP = 0x00010001;
    public static final int IOTYPE_USER_IPCAM_VSAAS_RECORD_PLAYCONTROL_REQ = 0x00010002;
    public static final int IOTYPE_USER_IPCAM_VSAAS_RECORD_PLAYCONTROL_RESP = 0x00010003;


    /* AVIOCTRL Event delete all  */    
	public static final int IOTYPE_VTECH_SET_DELETE_ALLEVENT_REQ = 0x779;
	public static final int IOTYPE_VTECH_SET_DELETE_ALLEVENT_RESP = 0x780;
	
	/* IOTYPE_VTECH_SET_DELETE_ALLEVENT_REQ 				= 0x779
	 */ 
	public static class SMsgAVIoctrlSetDeleteAllEventReq {
	...
	/* IOTYPE_VTECH_SET_DELETE_ALLEVENT_RESP					= 0x780
	 */ 
	public static class SMsgAVIoctrlSetDeleteAllEventResp {
		int result; // 1: success, 0: failure
	}


    /* Test sending audio file */
    public static final int IOTYPE_USER_IPCAM_PLAY_AUDIO_REQ = 0x0736;
    ...
    public static class SMsgAVIoctrlSetStreamCtrlReq {
        ...
        byte quality;        // refer to Quality Type
        byte[] reserved = new byte[3];
    ...
    public class SMsgAVIoctrlSetMotionDetectResp {
        byte result;     // 0: success; otherwise: failed.
        byte[] reserved = new byte[3];
    }
    ...
    public static class SMsgAVIoctrlDeviceInfoReq {
        ...
        static byte[] requestByte = new byte[20];

        public static byte[] parseContent () {
            SecureRandom random = new SecureRandom();
            random.nextBytes(requestByte);
            requestByte[0] = 0;
            requestByte[1] = 1;
            return requestByte;
        }
    ...
    public class SMsgAVIoctrlDeviceInfoResp {
        byte[] model = new byte[16];    // IPCam mode
        byte[] vendor = new byte[16];   // IPCam manufacturer
        int version;                    // IPCam firmware version   ex. v1.2.3.4 => 0x01020304;  v1.0.0.2 => 0x01000002
        int channel;                    // Camera index
        int total;                      // 0: No cards been detected or an unrecognizeable sdcard that could not be re-formatted.
    ...
    public class SMsgAVIoctrlDeviceInfoRespv1 {

	    int total;             // return total space size of nand flash (MBytes)
	    int free;              // free space size of nand flash (MBytes)
	    int sdcard_total;      // 0: No cards been detected or an unrecognizeable sdcard that could not be re-formatted.
	                                    // -1: if camera detect an unrecognizable sdcard, and could be re-formatted
	                                    // otherwise: return total space size of sdcard (MBytes)
	    int sdcard_free;       // Free space size of sdcard (MBytes)
	    
	    byte[] devicename = new byte[150];  // device Name
	    byte[] reserved = new byte[2];
    ...
    public static class SMsgAVIoctrlSetPasswdReq {
        byte[] oldPasswd = new byte[32];   // old password
        byte[] newPasswd = new byte[32];   // new password
    ...
    public static class SWifiAp {
        public byte[] ssid = new byte[32];   // SSID
        public byte mode;                    // refer to WiFi Mode
        public byte enctype;                 // AP's encode type, refer to WiFi Enc Type
        public byte signal;                  // signal intensity 0--100%
        public byte status;                  // 0 : invalid ssid or disconnected
    ...
    public static class SMsgAVIoctrlSetWifiReq {
        byte[] ssid = new byte[32];        // SSID
        byte[] password = new byte[32];    // AP's password
        byte mode;                         // refer to WiFi Mode
        byte enctype;                      // AP's encode type, refer to WiFi Enc Type
    ...
    public class SMsgAVIoctrlSetWifiResp {
        byte result;   // 0: wifi connected; 1: failed to connect
        byte[] reserved = new byte[3];
    }
    ...
    public static class SMsgAVIoctrlSetLenCorrectionReq {
        int channel;     // Camera Index
        int LencorrectionType;  // refer to correction Type
        byte[] reserved = new byte[4];
    ...
    public class SMsgAVIoctrlSetLenCorrectionResp {
        byte result;    // 0: success; otherwise: failed.
        byte[] reserved = new byte[3];
    }

    public static class SMsgAVIoctrlGetLenCorrectionReq {
        int channel; // Camera Index
        byte[] reserved = new byte[4];
    ...
    public static class SMsgAVIoctrlSetRecordReq {
        int channel;     // Camera Index
        int recordType;  // refer to Recording Type
        byte[] reserved = new byte[4];
    ...
    public static class SMsgAVIoctrlListEventReq {
        ...
        int channel;                          // Camera Index
        byte[] startutctime = new byte[8];    // start time of searching, UTC unit time by millisecond.
        byte[] endutctime = new byte[8];      // stop time of searching, UTC unit time by millisecond.
        byte event;                           // refer to Event Type
        byte status;                          // 0x00: Recording file exists, Event unreaded
    ...
    public static class SAvEvent {
        byte[] utctime = new byte[8];   // event time, UTC unit time by millisecond.
        byte event;                     // refer to Event Type
        byte status;                    // 0x00: Recording file exists, Event unreaded
                                        // 0x01: Recording file exists, Event readed
                                        // 0x02: No Recording file in the event
    ...
    public class SMsgAVIoctrlListEventResp {
        ...
        byte index;           // index of total
        byte endflag;         // end flag; endFlag = 1 means this package is the last one.
        byte count;           // how much events in this package
        byte reserved;
        SAvEvent[] stEvent;
    }

//For new list event
	public static class SMsgAVIoctrlNewListEventReq {
	...
	public static class SAvNewEvent {
	byte[] utctime = new byte[8];	// event time, UTC unit time by millisecond.
	byte event; 					// refer to Event Type
	byte status;					// 0x00: Recording file exists, Event unreaded
									// 0x01: Recording file exists, Event readed
									// 0x02: No Recording file in the event
	byte favourite;// 1: favourite. 0: not favourite									
	...
	public class SMsgAVIoctrlNewListEventResp {
	int channel;		  // Camera Index
	int total;			  // Total event amount in this search session
	byte index; 		  // index of total
	byte endflag;		  // end flag; endFlag = 1 means this package is the last one.
	...
	public static class SMsgAVIoctrlSetFavouriteEventReq {
		int channel; 	// Camera Index
		STimeDay stTimeDay; 	// Event time from ListEvent
		int favourite;
		byte[] reserved = new byte[4];

		public static byte[] parseContent (int channel, STimeDay stTimeDay, int fav) {

			byte[] result = new byte[20];
			byte[] ch = Packet.intToByteArray_Little(channel);
			byte[] favourite = Packet.intToByteArray_Little(fav);
			System.arraycopy(ch, 0, result, 0, 4);
			System.arraycopy(stTimeDay.toByteArray(), 0, result, 4, 8);
			System.arraycopy(favourite, 0, result, 12, 4);
			return result;
		}

	}

	public static class SMsgAVIoctrlSetFavouriteEventResp {
		public int result;	// 1: success; otherwise: failed.
		public SMsgAVIoctrlSetFavouriteEventResp(byte[] data) {
...
//For new list event
    ...
    public static class SMsgAVIoctrlPlayRecord {
        int channel;                      // Camera Index
        int command;                      // refer to Play Record Command
        int Param;
        byte[] stTimeDay = new byte[8];   // event time, refer to stTimeDay
        byte[] reserved = new byte[4];
    ...
    public class SMsgAVIoctrlPlayRecordResp {
        int command;      // refer to Play Record Command
        int result;       // when is AVIOCTRL_RECORD_PLAY_START:
                          //  result>=0   real channel for playback
                          //  result <0   error
                          //          -1  playback error
    ...
    public static class SMsgAVIoctrlSetEnvironmentReq {
        int channel;   // Camera Index
        byte mode;     // refer to Environment Mode
        byte[] reserved = new byte[3];
    ...
    public class SMsgAVIoctrlGetSupportStreamResp {
        int number;    // the quanity of supported audio&video stream or video stream
        SStreamDef[] streams;
    }

    public static class SStreamDef {

        public short index;   // the stream index of camera
        public short channel; // the channel index used in AVAPIs
        byte[] reserved = new byte[4];
...
//			byte[] channel_byte = channel_str.getBytes();
    ...
    public static class SMsgAVIoctrlGetAudioOutFormatReq {
        ...
        public static byte[] parseContent (int channel) {
            byte[] reserved = new byte[8];
            byte[] ch = Packet.intToByteArray_Little(channel);
            System.arraycopy(ch, 0, reserved, 0, 4);
    ...
    public class SMsgAVIoctrlGetAudioOutFormatResp {
        ...
        public int format;   // refer to Codec ID in AVFrame.java
    }
...
//			index += reserve.length;
    ...
    public static class SAvFile {
        ...
        byte status;                   // 0x00: Recording file exists, Event unreaded
                                       ...
                                       // 0x02: No Recording file in the event
        byte type;
        byte length;                   // length of path
        byte[] path = new byte[72];    // file's path
    ...
    public static class SMsgAVIoctrlSetVSaaSPlaybackReq{
        ...
        public static byte[] parseContent(String uid, int channel, int type){
            byte[] result = new byte[32];

            System.arraycopy(uid.getBytes(), 0, result, 0, uid.getBytes().length);
            byte[] nChannel = Packet.intToByteArray_Little(channel);
            byte[] nRecType = Packet.intToByteArray_Little(type);
            System.arraycopy(nChannel, 0, result, 20, 4);
            System.arraycopy(nRecType, 0, result, 24, 4);
    ...
    public static class SMsgAVIoctrlVSaaSPlayRecordReq{
        ...
        byte[] stTimeDay = new byte[8];         // Event time from ListEvent
        int offset;                             // assign offset (seconds) in the same recording file
        byte[] reserved = new byte[4];

        public static byte[] parseContent(int command, STimeDay stTimeDay, int offset){
            byte[] result = new byte[20];

            byte[] cmd = Packet.intToByteArray_Little(command);
            System.arraycopy(cmd, 0, result, 0, 4);
            System.arraycopy(stTimeDay.mBuf, 0, result, 4, 8);
            byte[] srcOffset = Packet.intToByteArray_Little(offset);
            System.arraycopy(srcOffset, 0, result, 12, 4);
    ...
    //    public static final int IOTYPE_USER_IPCAM_PLAY_AUDIO_REQ = 0x0736;
    public static class SMsgAVIoctrlPlayAudioReq {
        int nChannel;
        byte audioType; //0 300HZ; 1 1KHZ; 2 3KHZ
        byte sendStatus; //0 START; 1 END
        byte[] reserved = new byte[2];

        public static byte[] parseContent(int ch, byte type, byte status) {
            byte[] result = new byte[8];

            result[0] = (byte)ch;
            result[4] = (byte)type;
            result[5] = (byte)status;
